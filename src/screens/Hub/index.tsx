...
const PROFILE_ITEM = { path: '/profile', label: 'Profile', sub: 'Stats, achievements, skins', btnClass: 'btn-dark', icon: User };
const QUICK_PLAY_ITEMS = [
  {
    path: '/learn',
    label: 'Continue training',
    sub: 'Jump back into your drills',
    icon: BookOpen,
    background: '#102244',
  },
  {
    ...
    sub: 'Resume your campaign run',
    icon: Swords,
    background: '#0f2b1c',
  },
  {
    path: '/arena',
    label: 'Quick 5-player arena',
    sub: 'Hop into a fast 5-player match',
    icon: Trophy,
    background: '#3b1114',
  },
];
...
export function HubScreen() {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden felt-bg">
      {/* Header — full width */}
      <header className="relative z-10 px-4 lg:px-6" style={{ paddingTop: '18px', paddingBottom: '18px', height: 'fit-content' }}>
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between max-w-6xl mx-auto">
          {/* Logo */}
      ...
      {/* Main content — centered, wider on large desktop; two-column layout on lg+ */}
      <div className="flex-1 px-4 lg:px-6 w-full max-w-6xl mx-auto">
        <div className="lg:flex lg:gap-6 lg:items-start">
          {/* Left column on lg: user block + compact Profile */}
          <div className="lg:shrink-0 lg:w-80 flex flex-col gap-3">
            <motion.div
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.1 }}
              className="mb-4 p-3 balatro-panel lg:mb-0"
            >
              <div className="flex items-center gap-3">
            ...
            {/* Compact Profile under user block — large desktop only */}
            <motion.button
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.1 }}
              onClick={() => navigate(PROFILE_ITEM.path)}
              className="hidden lg:flex w-full items-center justify-between py-1.5 px-2.5 cursor-pointer text-left balatro-panel btn-balatro"
              style={{ fontFamily: 'inherit', textTransform: 'none', letterSpacing: 'normal' }}
              whileHover={{ x: 4 }}
              whileTap={{ x: 2, y: 2, boxShadow: 'none' }}
            >
              <span className="font-pixel text-xs leading-none" style={{ color: '#fff5d6' }}>{PROFILE_ITEM.label}</span>
              <span className="text-base font-pixel shrink-0" style={{ color: 'rgba(255,245,214,0.3)' }}>›</span>
            </motion.button>
          </div>

          {/* Menu items — single column on small, 2-col grid on lg+ (4 main items only) */}
          <div className="flex flex-col gap-3 lg:flex-1 lg:grid lg:grid-cols-2 lg:gap-3 lg:content-start">
            ...
            {/* Profile full-size — mobile/tablet only (hidden on lg when compact shows) */}
            <motion.button
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.1 }}
              onClick={() => navigate(PROFILE_ITEM.path)}
              className="lg:hidden w-full flex items-center gap-3 p-3 cursor-pointer text-left balatro-panel btn-balatro"
              style={{ fontFamily: 'inherit', textTransform: 'none', letterSpacing: 'normal' }}
              whileHover={{ x: 4 }}
              whileTap={{ x: 2, y: 2, boxShadow: 'none' }}
            >
              <div className={`w-11 h-11 rounded flex items-center justify-center shrink-0 ${PROFILE_ITEM.btnClass}`}
                style={{ minWidth: 44 }}>
                <User className="w-5 h-5 text-white" />
              </div>
              <div className="flex-1 min-w-0">
                <span className="font-pixel text-sm leading-none" style={{ color: '#fff5d6' }}>{PROFILE_ITEM.label}</span>
                <p className="text-xs mt-1" style={{ color: 'rgba(255,245,214,0.5)', fontFamily: 'Pixelify Sans' }}>{PROFILE_ITEM.sub}</p>
              </div>
              <span className="text-lg font-pixel" style={{ color: 'rgba(255,245,214,0.3)' }}>›</span>
            </motion.button>
          </div>
        ...
        {/* Desktop quick play row */}
        <div className="hidden lg:flex gap-4 pt-4">
          {QUICK_PLAY_ITEMS.map((item, index) => {
            const Icon = item.icon;
            return (
              <motion.button
                key={item.path}
                ...
                onClick={() => navigate(item.path)}
                className="flex-1 flex flex-col items-center justify-center cursor-pointer balatro-panel btn-balatro"
                style={{
                  fontFamily: 'inherit',
                  textTransform: 'none',
              ...
              >
                <div className="w-12 h-12 mb-2 rounded flex items-center justify-center shrink-0"
                  style={{ background: 'rgba(0,0,0,0.35)', border: '2px solid rgba(255,245,214,0.25)' }}>
                  <Icon className="w-6 h-6 text-white" />
                </div>
                <span className="font-pixel text-xs text-center leading-snug" style={{ color: '#fff5d6' }}>
                  {item.label}
                </span>
                <p className="text-[10px] mt-1 text-center" style={{ color: 'rgba(255,245,214,0.55)', fontFamily: 'Pixelify Sans' }}>
                  {item.sub}
                </p>
              </motion.button>
            );
